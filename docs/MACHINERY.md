...
Scratch tooling lives **outside the repo**, in the session scratchpad, so
a `tmp*.test.ts` can never be committed (they are `.test.ts` files and
*will* run in the suite, inflating counts).
