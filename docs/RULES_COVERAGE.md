...
- **Splice onto Arcane** (CR 702.47) for splices that need no targets: the card is revealed from hand and stays there, its cost joins the Arcane spell's in one payment, and its effects resolve after the spell's own.
...
- **Digging for a free cast**: Etali, Primal Storm exiles the top card of each library and lets its controller cast them for nothing; Etali, Primal Conqueror DIGS for each of them instead, exiling from the top until a nonland card turns up. Only the nonland cards are granted — the lands passed on the way stay exiled and uncastable, or the effect would hand one player everyone's land drop. An all-land library exiles itself and grants nothing.
- **Dynamic mana-value caps**: a search or dig filter can cap mana value by a board count ("with mana value less than or equal to the number of lands you control" — Loot, Exuberant Explorer), alongside the printed number, the announced X, and X plus a sacrificed creature. All four now resolve through one function at bind, so every consumer sees a plain number; a count the vocabulary cannot name refuses the descriptor rather than dropping the cap.
- **A blink returns the card under its OWNER's control** (CR 110.2a) — what comes back is a new object and nobody has taken control of it. Sword of Hearth and Home is printed to exploit exactly that: "exile up to one target creature you OWN" reaches one an opponent has stolen, and the blink brings it home. Ownership as a target filter is separate from control and is enforced on both the legality check and the choice enumeration.
- **Tapping another permanent as a mana cost** covers artifacts as well as creatures (Urza, Lord High Artificer beside Springleaf Drum), read through one `manaAbilityTapCost` so validation, availability, the costed-ability predicate and the fuzzer all ask the same question. An ability whose printed cost has no {T} no longer taps its own source, which had made Relic of Legends once per turn rather than repeatable.
- **One choice per player, all made by the caster**: a `choose_card` whose SOURCE names each player splits into one prompt per graveyard while the chooser stays the spell's controller (Breach the Multiverse), and each card arrives under that controller's control. The parked `resumeEffects` chain is what carries the second and third prompts, so the choices must be answered through `applyAction`. Portal to Phyrexia's "in addition to its other types" also works over a whole board, riding the instances — a creature that arrives afterwards is not covered.
- **Controlling an opponent's search** (Opposition Agent): the search prompt is answered by whoever controls the search, not whoever is searching, and it offers the searched player's zones. Everything found is exiled rather than reaching the printed destination, and the hijacker may play those cards for as long as they stay exiled — a grant cleanup leaves alone — spending mana as though it were any colour, which is the parsed cost with its coloured pips turned generic. Colorless pips are not covered, because colorless is not a colour.
- **Attacks are counted per card**, not just per player, so "gets +1/+0 for each time IT has attacked this turn" gives every creature its own bonus from one static (Moraug) — the layer engine already passes the affected card as the source of a `modify_pt` count. An added combat phase can owe an untap AT ITS BEGINNING rather than when the trigger resolved, which is the difference between Moraug and Aggravated Assault: the main phase in between is exactly the window the card does not give you.
- **Tibalt's Trickery**: counter a spell, then its controller mills a random one to three and digs to the first nonland card that does not share the countered spell's NAME, which they may cast for free; everything else exiled on the way goes to the bottom in a random order. The whole second half is one effect, and everything it says about "that spell" is read when the effects BIND — the counter beside it has already taken the spell away by the time it applies.
- **Chain of Vapor** builds out of pieces that already existed: a bounce, an optional choice over the bounced permanent's controller's lands, and the free spell copy Isochron Scepter makes. The copy belongs to the player who paid the land and picks its own target, so the chain travels round the table. "That player" in a choice's follow-up effects now means whoever answered, not only a player who declined.
- **A rule keyed to a counter, with no permanent to live on**: Promise of Loyalty leaves each player one creature carrying a vow counter, and those creatures cannot attack the caster or their planeswalkers for as long as the counter is there. The ban is game-level, read at attacker declaration, and ends when the counter does — not at end of turn, and not for the rest of the game.
- **Colour-restricted mana, both directions** (Throne of Eldraine): a mana ability can add several mana of the colour chosen as its source entered, spendable only on MONOCOLOURED spells of that colour — gold and colourless spells both fail it — and an activated ability can demand that its cost be paid in that colour alone, which is the printed cost with its generic turned into pips of it.
- **Bargain** (CR 702.166): an optional additional cost of sacrificing an artifact, enchantment, or token as you cast, recorded on the card while it is a spell so its own effects can ask. Beseech the Mirror is an ordinary tutor without it and gives the card away free — up to mana value four — with it. A search can now exile what it finds, and `lastSearchedCardIds` lets a later effect name it.
- **Necromancy** reads through the same pair of fields Animate Dead does, from the other end: an enchantment that becomes an Aura on the creature it reanimates. Cast when a sorcery could not have been, it is sacrificed at the next cleanup — recorded during the cast, because by cleanup nothing can tell what the timing was.
- **Spree bullets can carry riders** on the same printed line, each of which must compile in its own right (Return the Favor). "Target instant spell, sorcery spell, activated ability, or triggered ability" is a target kind of its own — wider than instant-or-sorcery, narrower than spell-or-ability — and both stack-object kinds now enumerate stack objects rather than falling through to players and creatures.
- **Bestow** (CR 702.103): a second cast offered beside the printed one, for the bestow cost, targeting the creature it will enchant. While attached it is an Aura and NOT a creature — a per-instance type change, beside the face-down one — and when its host leaves it comes loose as a creature rather than dying, which is a real exception to the loose-Aura state-based action and is checked before it.
...
- Top-of-library cast grants with a mana-value floor: "play lands and cast spells with mana value N or greater from the top of your library" (Glarb, Calamity's Augur) — lands stay unconditional, the floor gates the cast path.
- Capped untap steps: "players can't untap more than N lands/permanents during their untap steps" (Winter Orb, Static Orb) — a global symmetric cap imposed while the source is untapped; which permanents stay tapped is auto-picked (documented, never untaps more than the cap).
...
- Encore (CR 702.139): a sorcery-speed graveyard ability that exiles the card and makes a hasty token copy of it per opponent, sacrificed at the next end step (Impulsive Pilferer and other encore cards) — reuses Myriad-style per-opponent copies; the forced attack is a documented approximation.
...
- Wake the Past: "Return all <type> cards from your graveyard to the battlefield" now returns every card of that type from the controller's own graveyard at once (new return_all_from_graveyard effect), with the "they gain haste until end of turn" rider clearing summoning sickness on each. Distinct from mass_reanimate (all graveyards / creatures / control-steal) and return_all_lands (lands, tapped).
- Batched tribal enter trigger: "Whenever one or more other <Subtype> you control enter" compiles as a single once-per-batch watch (Elvish Warmaster), with the "This ability triggers only once each turn" cap riding it. Also fixed the tribal team-pump singularizer so irregular plurals (Elves->elf, Wolves->wolf) match real creatures instead of the non-existent "elve"/"wolve".
- "Each player loses N life" (Risky Shortcut) now compiles to an each_player lose_life — the symmetric counterpart to the already-handled "Each opponent loses N life".
- "Create ... token for each <Subtype> you control" (Elven Ambush, Krenko-style) now compiles: a Capitalized-subtype count tail on the general token-creation compiler, routed through singularSubtype and the existing perControlledSubtype applier.
- "This ability costs {1} less to activate for each <Subtype> you control" (Voldaren Estate) now discounts the activation per controlled creature of that subtype (new subtypeDiscount flag, the tribal cousin of legendaryDiscount).
- X-loyalty cost ("-X:" planeswalker abilities): the player announces X, pays X loyalty, and X flows to the effects as the bound xValue (Tezzeret the Seeker, Ugin, Vraska). New xLoyaltyCost flag; the loyalty resolution now threads xValue the way the activated-ability path already did.
- "Untap up to two target <permanents>" (Tezzeret the Seeker +1): the up-to-two-target clause handler now covers Untap alongside Exile/Destroy.
- "Artifacts you control become artifact creatures with base power and toughness N/N until end of turn" (Tezzeret the Seeker -5): a new animate_controlled_until_eot mass-animate — controlled permanents of one type gain the creature type (kept, not replaced) and a set base P/T until end of turn.
- "Each player draws X cards" (Prosperity) now compiles to an each_player draw of the announced X — the symmetric cousin of the numeric "each player draws N cards".
- "Whenever you activate an ability ... copy that ability" (Rings of Brighthearth): a new activated_ability trigger event + a copy_spell fromSubject form. The triggering ability is carried on the trigger as subjectStackObjectId so the copy resolves to the right stack object; retarget permission is approximated away.
- Station keyword + Station-threshold mana (Uthros, Titanic Godcore): the Station keyword compiles to a sorcery-speed "tap another creature you control: put charge counters equal to its power" ability (new costTapCreatureOther cost + add_counter amount sacrificed_power), and an "N+ | cost: Add {C} for each artifact you control" line compiles to a charge-gated per-artifact mana ability (new countFromArtifacts + requiresManaCounters on ManaAbility). Completed the Urza deck.
- "for each other creature you control" (Aragorn and Arwen, Wed) and "for each attacking creature" (Keep Watch): two new DynamicCount members (other_creatures_you_control excludes the source; attacking_creatures counts every attacker regardless of controller).
- "return target creature that player controls to its owner's hand" (Sigil of Sleep): a new controlledBySubject target scope, resolved at trigger time to the subject (damaged) player and enforced in violatesControlFilter. Also the "enchanted creature deals damage to a player" attached trigger head.
- "Activate only once each turn" (Wall of Roots, Three Tree Mascot): a new oncePerTurn cap on activated and mana abilities, enforced through oncePerTurnFired (throws on a second activation, dropped from legal actions / the mana-ability gate; cleared at turn start).
- Vivi Ornitier commander: "{0}: Add X mana in any combination of {U} and/or {R}, where X is its power" (no-tap mana ability, options_power + countFromPower, one-colour approximation) gated by "only during your turn" (onlyYourTurn) and "only once each turn"; plus a "put a +1/+1 counter on ~ and it deals N damage to each opponent" trigger body.
- Tergrid, God of Fright // Tergrid's Lantern: front face steals opponents' sacrificed nontoken permanents and discarded permanent cards onto the battlefield under your control (two watches reanimating the subject card); back face is a target-player "loses N life unless they sacrifice/discard" punisher (the each-opponent hailfire aimed at one chosen player).
- "Target player discards N cards" (a chosen-player discard), "Exile ~, then return it to the battlefield transformed" (self-transform for a DFC), and the "Magic" ability word before a cost. Reusable; +0 to the top-100 set.
- "... during your turn" trigger gate (new onlyYourTurn flag; Kefka, Ruler of Ruin) and the each-opponent sacrifice edict extended to "a permanent of their choice".
- Kefka, Court Mage: "each player discards a card, then you draw a card for each card type among cards discarded this way" — a new discard_each_draw_per_type effect (synchronous, since discards auto-pick), drawing one per DISTINCT card type among the discards. Completes the commander together with wave 459.
- Kuja, Genome Sorcerer // Trance Kuja, Fate Defied (a MDFC): a token created "with '<triggered ability>'" now carries those abilities on its own definition (new create_token `tokenTriggers`), the "Then if you control four or more Wizards, transform ~" muster clause folds on as an if_condition self-transform, and the back face’s Wizard-scoped damage doubling ("Flare Star — If a Wizard you control would deal damage …, it deals double") introduces a DamageReplacement `sourceSubtype` gate plus ability-word-prefix stripping. Flipped Kuja — Kefka, Court Mage 3 -> 2 broken.
- Tinybones, Bauble Burglar: the "stash" mechanic — a discards/opponents trigger exiles the just-discarded card from its owner’s graveyard with a `stash` counter (new `stash_exile_grant` effect) and grants its controller a standing play-from-exile permission (`exilePlayable` with `whileExiled` + `anyColorMana`), so the stolen card can be cast from exile paying mana of any type; the descriptive "During your turn, you may play ..." static compiles to a `playExiledWithStashCounters` marker. Flipped Tinybones — Kefka, Court Mage 2 -> 1 broken.
- Kefka, Dancing Mad (Kefka, Court Mage’s last card, completing the deck): the LEADING-form your-turn keyword static ("During your turn, ~ has indestructible", a requiresYourTurn grant) — plus a fix to `keywordsFromOracle` so a first-line "during your turn" clause no longer leaks its keyword in as an unconditional base keyword — and the end-step graveyard heist: a new `exile_gy_random_free_cast` effect exiles one random card from each opponent’s graveyard, grants a free cast of each (`exilePlayable` freeCast + a new `ownerLosesLifeManaValue` punisher that makes the card’s owner lose life equal to its mana value when it is cast). ★ 3rd deck complete (Kefka, Court Mage).
- Flashback (the card, a Snapcaster-style grant): a new instance `flashbackUntilEot` flag (a temporary flashback grant, cost = the card’s own mana cost, cleared at cleanup) read alongside the printed `definition.flashback` at every cast site (viaFlashback/fromFlashback/legalActions offering/graveyard filter, so the exile-on-resolve still fires); the `grant_flashback_until_eot` effect targets an instant or sorcery card in your graveyard. Also fixes `normalizeOracleText` so a card literally NAMED after a keyword (Flashback, Foretell, Cascade) no longer has that word rewritten to ~ as a self-reference. Flipped Flashback — Vivi Ornitier 4 -> 3 broken.
- Wizard’s Staff: three equipment pieces — "Equipped creature has prowess" (parseGrantPredicate expands "prowess" to a grant_trigger of the full prowess rules text, handed to the attached creature), a new `triggerDoubling.source.attached` flag (the doubled ability’s source must be the Equipment’s attached creature), and "Equip <Subtype> {N}" (a cheaper subtype-restricted equip via own_creature + requiredSubtypes). The equipped creature’s granted prowess is itself doubled by the same Staff (+2/+2 per noncreature spell). Flipped Wizard’s Staff — Vivi Ornitier 3 -> 2 broken.
- Quicksilver Elemental: "{U}: gains all activated abilities of target creature until end of turn" — a new `gain_all_activated_of_target` effect copies the target’s activatedOf onto a new instance list `grantedActivatedUntilEot` (appended by activatedOf, cleared at cleanup), so the copies belong to Quicksilver and their "self" resolves to it. Plus a `spendBlueAsAnyForAbilities` definition flag (blue pays any colour in this permanent’s own ability costs, honoured in applyActivateAbility). Flipped Quicksilver — Vivi Ornitier 2 -> 1 broken.
- Soulbond (Tandem Lookout, completing the Vivi deck): a new pairing mechanic — instance `soulbondPartner` + definition `soulbond`, paired greedily in `pairSoulbondInPlace` on every enter (stale pairings dropped first, which is how a pair ends when you stop controlling both). "As long as ~ is paired with another creature, each of those creatures has '<ability>'" grants the quoted ability through a new `soulbond_pair` selector scope (the source and its live partner), which is itself the while-paired gate. ★ 4th deck complete (Vivi Ornitier).
- Annie Joins Up (broken in 5 top-100 decks): "it deals N damage to target ..." now compiles (the three deals-to-target handlers accept an "it" subject, the ETB source, beside "~"), and a new `triggerDoubling.source.legendary` flag doubles the triggered abilities of legendary creatures you control (Annie’s second half). Reusable across every legendary-doubler and "it deals" ETB; universe 1773 -> 1774.
- Hallowed Spiritkeeper (nearest-deck grind, Teysa): "create X tokens ... where X is the number of creature cards in your graveyard" — a new generic `create_token.perDynamicCount` (any DynamicCount, resolved at bind via dynamicCountOf) plus the graveyard-count alternative in the token-count regex, so the dies trigger scales its Spirit tokens off the graveyard (counting itself). Flipped Spiritkeeper — Teysa Karlov 7 -> 6 broken.
- Rite of Oblivion (Teysa grind): the additional cast cost "sacrifice a nonland permanent" — a new `nonland_permanent` member of AdditionalCastCost.sacrifice (parser + serialize validator; the cast-time sacrificeScopeMatches already knew the scope). The exile and Flashback already compiled. Flipped Rite — Teysa Karlov 6 -> 5 broken.
- Teysa, Orzhov Scion (Teysa grind): a colour-filtered activation cost ("Sacrifice three WHITE creatures", a new ActivatedAbility.sacrificeColor threaded through the cost regexes / parser / builders / activation validation + fodder / serialize) and a colour-filtered trigger subject ("another BLACK creature you control dies" — parseTriggerSubjectPhrase strips a colour adjective into subjectFilter.colors). The colour group sits inside the count branch so "another black creature" keeps its dedicated scope. Flipped Teysa Scion — Teysa Karlov 5 -> 4 broken.
- Enhanced Surveillance (Glarb grind): a `surveilLookBonus` definition flag (summed from your permanents in applySurveil, so "look at an additional two cards each time you surveil" deepens every surveil) and an exile-self activated ability that shuffles your graveyard into your library. Fixes a shared gap along the way: parseAbilityCost now accepts an exile-self-only cost (the guard omitted exileSelfCost, so Nyx Weaver-class abilities never compiled). Flipped Enhanced Surveillance — Glarb 7 -> 6 broken.
- Lilysplash Mentor (Glarb grind): the "then return it ... with a +1/+1 counter on it" tail on a self-blink — a new `withCounter` rider on the flicker effect (captured in the return regex, threaded through binder / applier / serialize), so a blinked creature can come home carrying a counter. Flipped Lilysplash — Glarb 6 -> 5 broken.
- Colossal Grave-Reaver (Glarb grind): "one or more creature cards are put into your graveyard from your library, put one of them onto the battlefield" — the graveyard_from_elsewhere trigger head now also reads "from your library", and two new bodies reanimate the milled subject ("put one of them onto the battlefield") or return it tapped ("put them onto the battlefield tapped"). Flipped Reaver (and a second card sharing the bodies) — Glarb 5 -> 4 broken.
- Grolnok, the Omnivore (Glarb grind): a croak-counter recursion engine on the exilePlayable machinery — a graveyard_from_elsewhere trigger on a milled PERMANENT card (nonTypes instant/sorcery) whose new `croak_exile_grant` effect exiles the subject with a croak counter and a whileExiled play-from-exile grant for its owner, plus a `playExiledWithCroakCounters` marker for the descriptive static. Flipped Grolnok — Glarb 4 -> 3 broken.
- Crew (Vehicle keyword, previously unimplemented — flips Hedge Shredder plus six more Vehicles): "Crew N" is an activated ability that taps a creature supplying the crew power (Station’s costTapCreatureOther + a new `crewPower`) to make the Vehicle an artifact creature until end of turn (new `become_creature_until_eot`, an until-EOT add_types so the printed P/T switch on). Also adds "this Vehicle" to normalizeOracleText’s self-reference list. Universe +7; Glarb 3 -> 2 broken.
- Counterbalance (Glarb grind): a new `counterbalance` effect — reveal the top card of your library and counter the just-cast opponent spell only if the two share a mana value — bound from the cast trigger’s subject. Cast triggers now thread subjectStackObjectId (the just-cast spell’s stack object), not only for activated_ability events, so "counter that spell" has a handle. Flipped Counterbalance — Glarb 2 -> 1 broken.
- Muldrotha, the Gravetide (completing the Glarb deck): "play a land and cast a permanent spell of each permanent type from your graveyard" — a `castPermanentsFromGraveyard` static grant plus a per-turn `graveyardTypesPlayedThisTurn` record over the five permanent types, wired through derived (canCastPermanentsFromGraveyard / muldrothaTypeAvailable), legalActions, applyCastSpell + applyPlayLand (record the spent type), putSpellOnStack (accept the source), and the turn-start reset. ★ 5th deck complete (Glarb, Calamity’s Augur).
- Morph / Megamorph (CR 702.37 / 702.108) — grinding the Teysa and Animar decks: "Morph {cost}" compiles to a `morph` definition field; a `cast_face_down` action puts a morph card onto the battlefield face down as a 2/2 for {3} (a documented approximation that resolves without using the stack), and turn_face_up pays the morph cost (plus a +1/+1 counter for megamorph) and emits a `turns_face_up` event that "When ~ is turned face up, ..." triggers react to. Flipped Grim Haruspex (Teysa 4 -> 3) and Rattleclaw Mystic (Animar 9 -> 8).
- Priest of Forgotten Gods (grinding the Teysa deck): the sac-two-other-creatures activated cost already parsed; added the two effect sentences — "Any number of target players each lose N life and sacrifice a creature of their choice" (each_opponent life loss + the each-opponent edict, a documented approximation of the player choice) and "You add {mana} and draw a card" (add_mana + draw). Flipped Priest of Forgotten Gods — Teysa 3 -> 2.
- Athreos, God of Passage (grinding the Teysa deck): widened the Theros-god devotion gate (`notCreatureBelowDevotion`) from a single colour to a colour LIST so "devotion to white and black" reads, with devotionPips counting a hybrid symbol once; and added the "Whenever another creature you own dies" trigger head plus the "return it to your hand unless target opponent pays N life" punisher body (an unless_pays that returns the dead subject unless the targeted opponent pays the life). Flipped Athreos — Teysa 2 -> 1 (only Sephiroth remains).
- Sephiroth, Fabled SOLDIER — part 1 (grinding the Teysa deck, its last card): the front face’s "if this is the Nth time this ability has resolved this turn, transform ~" rider now compiles — a per-source per-turn `abilityResolutionsThisTurn` count and an `nth_resolution_transform` effect, folded onto the dies trigger via foldSubjectRider. The card does not flip yet: its back face still needs a variable "sacrifice any number, draw that many" and a transform-triggered emblem carrying a dies-drain.
